    ...
    wayland.windowManager.hyprland = {
        ...
        settings = {
            ...
            bind = [
                ...
                "ALT_L, F9, exec, killall -SIGINT gpu-screen-recorder && gsr-notify --text 'Recording ended' --timeout 3 --icon record || (gsr-notify --text 'Recording started' --timeout 3 --icon record && gpu-screen-recorder -w \"$(hyprctl monitors -j | jq -r '.[] | select(.focused) | .name')\" -c mp4 -ac opus -cursor yes -cr full -fm cfr -k ${videoCodec} -encoder gpu -f 60 -v no -o ~/Videos/Temporary/Video_$(date +%Y-%m-%d_%H-%M-%S).mp4 -bm cbr -q 40000 -a app-inverse: -restore-portal-session yes)"
